...
CSV.parse(S3_CS_BUCKET.objects['xactly/data/xactly.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
    ...
    if Rails.configuration.x.load_images
      begin
        product.images << Spree::Image.create!(
          attachment: open(S3_CS_BUCKET.objects["xactly/data/product_images/#{product_attrs[:sku]}.jpg"].public_url),
          viewable: product
        )
      rescue => e
...
# UPCHARGES
CSV.parse(S3_CS_BUCKET.objects['xactly/data/xactly_upcharges.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
...
CSV.parse(S3_CS_BUCKET.objects['xactly/data/xactly_preconfigure.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
