...
main_color_map = ProductLoader.main_color_map_load('db/product_data/bullet_main_color_map.csv')

supplier = Spree::Supplier.where(name: 'Bullet').first_or_create

# PMS Colors
ProductLoader.pms_load('bullet_pms_colors.csv', supplier.id)
...
CSV.foreach(file_name, headers: true, header_converters: :symbol) do |row|
  ...
  begin
    ...
    if Rails.configuration.x.load_images
      begin
        image_path = File.join(Rails.root, "db/product_images/bullet/#{product_attrs[:sku]}.jpeg")
...
file_name = File.join(Rails.root, 'db/product_data/bullet_pricing.csv')
CSV.foreach(file_name, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
