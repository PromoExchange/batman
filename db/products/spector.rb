...
puts 'Loading Spector Products'

# Load the main color map
main_color_map = ProductLoader.main_color_map_load('db/product_data/spector_main_color_map.csv')

supplier = Spree::Supplier.where(name: 'Spector').first_or_create

# PMS Colors
ProductLoader.pms_load('spector_pms_colors.csv', supplier.id)
...
file_name = File.join(Rails.root, 'db/product_data/spector.csv')
load_fail = 0
image_fail = 0
count = 0
...
CSV.foreach(file_name, headers: true, header_converters: :symbol) do |row|
  ...
  begin
    ...
    if Rails.configuration.x.load_images
      begin
        image_path = File.join(Rails.root, "db/product_images/spector/#{product_attrs[:sku]}.jpeg")
...
file_name = File.join(Rails.root, 'db/product_data/spector_pricing.csv')
CSV.foreach(file_name, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
