...
RSpec.describe Spree::Quote, type: :model do
  ...
  xit 'should apply EQP discount' do
  ...
  xit 'should not apply no_eqp_range' do
  ...
  xit 'should apply pricing discount' do
  ...
  xit 'should apply seller markup' do
    quote = FactoryGirl.create(:quote)
    quote.unit_price = quote.product.unit_price(quote.quantity)
  ...
  xit 'should apply PX commission' do
    quote = FactoryGirl.create(:quote)
    quote.unit_price = quote.product.unit_price(quote.quantity)
  ...
  xit 'should apply processing fee' do
    quote = FactoryGirl.create(:quote)
    quote.unit_price = quote.product.unit_price(quote.quantity)
  ...
  xit 'should calculate quote with 25 quantity', focus: true do
    ...
    expect((quote.total_price - 767.281).abs).to be < 0.001
  end

  xit 'should calculate quote with 125 quantity', focus: true do
    quote = FactoryGirl.create(:quote, quantity: 125)
    expect((quote.total_price - 3504.749).abs).to be < 0.001
  end

  xit 'should calculate quote with 225 quantity', focus: true do
    quote = FactoryGirl.create(:quote, quantity: 225)
    expect((quote.total_price - 5864.272).abs).to be < 0.001
  end

  xit 'should apply product less_than_minimum charge', focus: true do
    quote2 = FactoryGirl.create(:quote, :with_less_than_minimum)
    expect((quote2.total_price(quantity: 75) - 841.966).abs).to be < 0.001
  ...
  xit 'should apply product setup and run charges with quantity' do
    quote2 = FactoryGirl.create(:quote, :with_setup_and_run_upcharges)
    quote2.quantity = 125
    expect((quote2.total_price - 3810.959).abs).to be < 0.001
    expect(quote2.total_price).to be > quote.total_price
  end
  ...
  xit 'should apply additional location charge' do
    ...
    expect((quote2.total_price - 769.771).abs).to be < 0.001
    expect(quote2.total_price).to be > quote.total_price
  end
  ...
  xit 'should apply additional location charge with quantity' do
    ...
    quote2.num_locations = 2
    expect((quote2.total_price - 3514.707).abs).to be < 0.001
    expect(quote2.total_price).to be > quote.total_price
  end
  ...
  xit 'should use non fixed shipping 3day' do
    quote = FactoryGirl.create(:quote, :with_carton)
    expect((quote.total_price(shipping_option: :ups_3day_select) - 794.218).abs).to be < 0.001
  end
