...
CSV.parse(S3_CS_BUCKET.objects['hightail/data/hightail.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
...
# UPCHARGES
CSV.parse(S3_CS_BUCKET.objects['hightail/data/hightail_upcharges.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
...
# PRECONFIGURE
puts 'Loading Hightail preconfigures'

CSV.parse(S3_CS_BUCKET.objects['hightail/data/hightail_preconfigure.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash

  product = Spree::Product.joins(:master).where("spree_variants.sku='#{hashed[:sku]}'").first

  buyer = Spree::User.where(email: 'lindsey.robinson@hightail.com').first
  fail "Unable to locate Hightail user" if buyer.nil?
