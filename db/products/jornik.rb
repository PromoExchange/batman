...
puts 'Loading Jornik products'

# Load the main color map
main_color_map = ProductLoader.main_color_map_load('db/product_data/jornik_main_color_map.csv')

supplier = Spree::Supplier.where(name: 'Jornik').first_or_create

# PMS Colors
ProductLoader.pms_load('jornik_pms_colors.csv', supplier.id)
...
file_name = File.join(Rails.root, 'db/product_data/jornik.csv')
load_fail = 0
image_fail = 0
count = 0
...
CSV.foreach(file_name, headers: true, header_converters: :symbol) do |row|
  ...
  begin
    ...
    if Rails.configuration.x.load_images
      begin
        image_path = File.join(Rails.root, "db/product_images/jornik/#{product_attrs[:sku]}.png")
...
file_name = File.join(Rails.root, 'db/product_data/jornik_pricing.csv')
CSV.foreach(file_name, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
