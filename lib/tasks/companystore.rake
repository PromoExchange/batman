def create_company_store(email, name, display_name, slug)
  ...
  raise 'failed to find user' if user.nil?
...
namespace :companystore do
  ...
  task xactly: :environment do
    company_store = create_company_store(
      'Xactly Company Store',
      'xactly_cs@thepromoexchange.com',
      'Xactly',
      'xactly'
    )
    products = [
      { query: { dc_acct_num: '100306' }, skus: ['XA-8150-85', 'XA-7120-15'] }, # Leeds
      { query: { dc_acct_num: '100383' }, skus: ['XA-SM-4125'] }, # Bullet
      { query: { dc_acct_num: '100257' }, skus: ['XA-40061'] }, # Gemline
      { query: { name: 'River\'s End Trading' }, skus: ['XA-6223'] }, # Rivers End Trading
      { query: { name: 'American Apparel' }, skus: ['XA-F497', 'XA-2001-WHITE'] }, # American Apparel
      { query: { dc_acct_num: '100108' }, skus: ['XA-5117'] }, # Innovation line
      { query: { name: 'Next Level' }, skus: ['XA-6240-SXL', 'XA-6640-SXL'] }, # Next Level
      { query: { dc_acct_num: '101760' }, skus: ['XA-PD46P-25'] }, # 3M
      { query: { dc_acct_num: '101044' }, skus: ['XA-BA2300'] }, # Logomark
      { query: { dc_acct_num: '101371' }, skus: ['XA-BTR8'] }, # Digispec
      { query: { dc_acct_num: '446654' }, skus: ['XA-CPP5579'] }, # Cloth Promotions
      { query: { name: 'Alpi International' }, skus: ['XA-20099'] } # Alpi International
    ]
    load_products('xactly', store_name)
  ...
  task anchorfree: :environment do
    company_store = create_company_store(
      'Anchorfree Company Store',
      'anchorfree_cs@thepromoexchange.com',
      'Anchorfree',
      'anchorfree'
    )
    products = [
    ...
    load_products('anchorfree', store_name)
    assign_original_supplier(products)
    create_price_cache(company_store.supplier)
  end
  ...
  task hightail: :environment do
    company_store = create_company_store(
      'Hightail Company Store',
      'hightail_cs@thepromoexchange.com',
      'Hightail',
      'hightail'
    )
    products = [
    ...
    load_products('hightail', store_name)
    assign_original_supplier(products)
    create_price_cache(company_store.supplier)
  end
  ...
  task netmining: :environment do
    company_store = create_company_store(
      'Netmining Company Store',
      'netmining_cs@thepromoexchange.com',
      'Netmining',
      'netmining'
    )
    products = [
    ...
    load_products('netmining', store_name)
    assign_original_supplier(products)
    create_price_cache(company_store.supplier)
  end
  ...
  task pimco: :environment do
    company_store = create_company_store(
      'PIMCO Company Store',
      'pimco_cs@thepromoexchange.com',
      'PIMCO',
      'pimco'
    )
    load_products('pimco', store_name)
