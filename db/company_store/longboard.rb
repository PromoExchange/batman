...
CSV.parse(S3_CS_BUCKET.objects['longboard/data/products.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
    ...
    if Rails.configuration.x.load_images
      begin
        product.images << Spree::Image.create!(
          attachment: open(S3_CS_BUCKET.objects["longboard/data/product_images/#{product_attrs[:sku]}.jpg"].public_url),
          viewable: product
        )
      rescue => e
...
# UPCHARGES
puts 'Loading Longboard upcharges'
CSV.parse(S3_CS_BUCKET.objects['longboard/data/upcharges.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash
...
# PRECONFIGURE
puts 'Loading Longboard preconfigures'
CSV.parse(S3_CS_BUCKET.objects['longboard/data/preconfigure.csv'].read, headers: true, header_converters: :symbol) do |row|
  hashed = row.to_hash

  product = Spree::Product.joins(:master).where("spree_variants.sku='#{hashed[:sku]}'").first
  buyer = Spree::User.where(email: 'kate@longboard-am.com').first
